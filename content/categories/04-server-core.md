---
id: server-core
name: 服务端核心
icon: 🧱
description: Paper、Folia、Velocity、Forge、Fabric 等 Minecraft 服务端核心与代理端入口。
links:
  - title: Leaf
    url: https://github.com/Winds-Studio/Leaf
    description: Purpur+Gale的下游 专注性能提升 尊重原版行为、兼容插件生态
    tags: [server, core, github]
  - title: LightingLuminol
    url: https://luminolsuki.moe/
    description: Luminol下游 目标是在Folia上运行更多Bukkit插件
    tags: [server, core, github]      
  - title: Lophine
    url: https://luminolsuki.moe/
    description: Luminol下游 目标是在Folia上实现更多生电的内容
    tags: [server, core, github]    
  - title: Luminol
    url: https://luminolsuki.moe/
    description: Folia下游 专为生存和无政府服务器设计并提供丰富配置与的 API 支持
    tags: [server, core, github]
  - title: Uranium
    url: https://starleap.lanzout.com/iPrLI1v1e1dc
    description: 1.7.10最优秀的混合端 已停更
    tags: [server, core, download]
  - title: Hexcord
    url: https://github.com/CronixMC/HexaCord
    description: 低版本mod服适用的bungeecord 修复了一些模组产生的问题
    tags: [proxy, server, github]
  - title: Arclight
    url: https://github.com/IzzelAliz/Arclight
    description: 优秀的混合端 支持fabric+bukkit 或 forge+bukkit
    tags: [server, forge, bukkit, github]
  - title: Sponge
    url: http://www.spongepowered.org/
    description: 原生spongeapi 支持forge 不支持bukkit
    tags: [server, sponge]
  - title: Leaves
    url: https://leavesmc.org/
    description: Paper下游 旨在修复被破坏的原版特性。
    tags: [server, core]
  - title: Catserver
    url: https://catmc.org/
    description: 老牌 优秀的Bukkit+Forge端 已停更
    tags: [server, core]
  - title: Purpur
    url: https://purpurmc.org/
    description: 主流Bukkit服务端 高性能 推荐
    tags: [server, paper, core]
  - title: Youer
    url: https://www.mohistmc.cn/resources/Youer
    description: Neoforge 并完整兼容 Bukkit/Spigot/Paper/Purpur API。
    tags: [server, core, mohist]    
  - title: Silkard
    url: https://www.mohistmc.cn/resources/silkard
    description: Fabric下游 兼容Bukkit、Spigot.
    tags: [server, core, mohist]
  - title: Mohist
    url: https://www.mohistmc.cn/resources/mohist
    description: Forge下游 兼容Bukkit、Spigot.
    tags: [server, forge, bukkit]
  - title: Velocity
    url: https://papermc.io/software/velocity
    description: 新时代代理端 群组核心 高性能 推荐 
    tags: [proxy, papermc, server]
  - title: Foila
    url: https://papermc.io/software/folia
    description: 新时代核心 支持多线程 不支持原生Bukkit插件
    tags: [server, papermc, folia]
  - title: Paper
    url: https://papermc.io/downloads/paper
    description: 主流Bukkit核心
    tags: [server, papermc, paper]
  - title: Bungeecord
    url: https://www.spigotmc.org/wiki/bungeecord/
    description: 老牌群组核心 已过时 不再推荐
    tags: [proxy, spigot, server]
  - title: Spigot
    url: https://getbukkit.org/download/spigot/
    description: 原版插件核心 不推荐 优化很差
    tags: [server, bukkit, spigot]
  - title: Forge
    url: https://files.minecraftforge.net/net/minecraftforge/forge/
    description: 访问 Minecraft Forge 下载页面。
    tags: [modding, forge, server]
  - title: Fabric
    url: https://fabricmc.net/use/server/
    description: 访问 Fabric 服务端使用页面。
    tags: [modding, fabric, server]
  - title: Vanilla
    url: https://getbukkit.org/download/vanilla
    description: 官方核心 不支持forge fabric bukkit等
    tags: [server, vanilla]
---
