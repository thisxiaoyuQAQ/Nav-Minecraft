...
function RepoLink() {
  const label = __LAST_COMMIT_DATE__ ? `GitHub 仓库，最后提交 ${__LAST_COMMIT_DATE__}` : 'GitHub 仓库'
  ...
  return (
    <a
      ...
      rel="noreferrer"
      aria-label={label}
      title={__LAST_COMMIT_DATE__ ? `最后提交：${__LAST_COMMIT_DATE__}` : undefined}
    >
      ...
      {__LAST_COMMIT_DATE__ && <span>最后提交 {__LAST_COMMIT_DATE__}</span>}
    </a>
  )
}
